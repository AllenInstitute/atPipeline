...
#channelnames = ["2_DAPI_2", "2_GluN1", "2_TdTomato", "2_GAD2"]
...
#	print "DAPI has been rough aligned"
 #   else:
	...
	cmd_cons = cmd_cons + "--stack Deconvolved_%s --output_stack Deconvolved_%s_CONS --postfix CONS  "%(ch,ch)
	...
	cmdappstitch = "docker exec renderapps_develop python -m renderapps.registration.apply_transforms_by_frame %s %s --cyclenumber 200 --alignedStack Stitched_1_DAPI_1_dropped --inputStack Deconvolved_%s --outputStack Stitched_deconv_%s_dropped --pool_size 20"%(render_str,project_str,ch,ch)
        cmd_ch = "%s python -m renderapps.rough_align.ApplyLowRes2HighRes %s %s %s "%(d_str,render_str,project_str,applych_str)
        cmd_ch = cmd_ch + "--input_stack Deconvolved_%s --output_stack Rough_Aligned_Deconvolved_%s --tilespec_directory %s_%s_deconv "%(ch,ch,roughalign_ts_dir,ch)
        cmd_ch = cmd_ch + "--minZ %d --maxZ %d "%(firstribbon*100, (lastribbon+1) * 100)
...
for ch in channelnames:
	#ch = "PSD95"
         	...
         	#print "Do Nothing"
	#else:
	cmdappalign = "docker exec renderapps_develop python -m renderapps.registration.apply_transforms_by_frame %s %s --cyclenumber 200 --alignedStack  Fine_Aligned_DAPI_1_filtered --inputStack  Rough_Aligned_Deconvolved_%s --outputStack Fine_Aligned_Deconvolved_%s_filtered --pool_size 20"%(render_str,project_str,ch,ch)
...
#for i in range (0,1):
#	ch = "3_MBP"
	cmdappalign = "docker exec renderapps_develop python -m renderapps.registration.apply_alignment_transform_from_registered_stack %s %s --prealigned_stack Rough_Aligned_1_DAPI_1_CONS --postaligned_stack  Fine_Aligned_DAPI_1 --source_stack  Rough_Aligned_Deconvolved_%s --output_stack Fine_Aligned_Deconvolved_%s --pool_size 20"%(render_str,project_str,ch,ch)
