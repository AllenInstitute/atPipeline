	...
	for i in range (0,1):
			...
			if not os.path.exists(flatfield_dirname):
				...
				for ch in channels:
					...
					deconv_stack = "DCV_S0%d_%s"%(int(session),ch)
					...
					if ch=="GFP":
						#print "Im gfp"
						psf_file = "/nas5/data/M367240B_SSTPV_Lamin_A/processed/psfs/psf_LaminA.tif"
						scale_factor = 5
						bgrd_size = 20
					if "DAPI" in ch:
					#	#print "im dapi"
						psf_file = "/nas5/data/M367240B_SSTPV_Lamin_A/processed/psfs/psf_DAPI.tif"
                                                scale_factor = 2
                                                bgrd_size = 20
					#elif ch=="GABA":
					#	#print "im geph"
				...
				for ch in channels:
					if ch.find("DAPI")>-1:
						...
						flatfield_stack = "FF_S0%d_%s"%(int(session),ch)
						stitched_stack = "STI_S0%d_%s"%(int(session),ch)
						stitched_deconv_stack = "STI_DCV_S0%d_%s"%(int(session),ch)
