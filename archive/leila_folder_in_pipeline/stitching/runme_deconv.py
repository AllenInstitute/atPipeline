...
if __name__ == "__main__":
	...
	for i in range (0,1):
			...
			if not os.path.exists(flatfield_dirname):
				...
				dcmd = "docker exec renderapps_olga python -m renderapps.dataimport.create_fast_stacks "
				...
				dcmd = dcmd + "--render.project %s "%project
 				...
 				dcmd = dcmd + "--outputStackPrefix Acquisition_%d "%(session)
				...
				for ch in channels:
					...
					savemedianjson(med,medianfile,owner, project,acq_stack,median_stack,median_dir,z,z,close_stack)
					...
					#run
					mystr = "DAPI"
					cmd1 = "docker exec rm_container python -m rendermodules.intensity_correction.calculate_multiplicative_correction --input_json %s"%medianfile
					cmd2 = "docker exec rm_container python -m rendermodules.intensity_correction.apply_multiplicative_correction --input_json %s"%flatfieldfile
					cmd3 = "docker exec renderapps_develop python -m renderapps.intensity_correction.apply_deconvolution --input_json %s"%deconvfile
				...
				for ch in channels:
					if ch.find("DAPI")>-1:
						...
						stitched_dapi_stack = "Stitched_%d_DAPI_%d"%(int(session),int(session))
						flatfield_stack = "Flatfieldcorrected_%d_%s"%(int(session),ch)
						stitched_stack = "Stitched_%d_%s"%(int(session),ch)
						...
						cmdapp = "docker exec renderapps_testsharmi python -m renderapps.registration.apply_transforms_by_frame --input_json %s"%applystitchingfile
						#os.system(cmdapp)
						#f.write(cmdapp+"\n")
