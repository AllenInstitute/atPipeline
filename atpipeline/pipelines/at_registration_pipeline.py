            ...
            try:
                ...
                if session == reference_session:
                    ...
                    for sectnum in range(firstSection, lastSection + 1):
                        ...
                        cmd = cmd + " --grossRefStack %s"               %("tempGrossStack1" % rp.project_name[:25])
                        cmd = cmd + " --grossStack %s"                  %("tempGrossStack2" % rp.project_name[:25])
                        cmd = cmd + " --filter %d"                      %(1)
                        cmd = cmd + " --matchcollection %s"             %("%s_S1_S%d_matches" % (rp.project_name[:25], session))
