...
if __name__ == "__main__":
	...
	for i in range (0,1):
			...
			if not os.path.exists(flatfield_dirname):
				...
				dcmd = "docker exec renderapps_multchan python -m renderapps.dataimport.create_fast_stacks "
				...
				for ch in channels:
					...
					#run
					mystr = "DAPI"
					cmd1 = "docker exec renderapps_multchan python -m rendermodules.intensity_correction.calculate_multiplicative_correction --render.port 8988 --input_json %s"%medianfile
					cmd2 = "docker exec renderapps_multchan python -m rendermodules.intensity_correction.apply_multiplicative_correction --render.port 8988 --input_json %s"%flatfieldfile
