...
def run(p, sessionFolder):
	...
	# docker commands
	cmd_drop = "%s python -m renderapps.stitching.detect_and_drop_stitching_mistakes %s %s %s %s"%(d_str,render_str, project_str, output_str, params_str)
	print cmd_drop
	# os.system(cmd_drop)
	# exit(0)

	# Run =============
	print ("Running: " + cmd_drop)

	proc = subprocess.Popen(cmd_drop, shell=True, stdout=subprocess.PIPE, stderr=subprocess.STDOUT)
